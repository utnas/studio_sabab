...
StudioSabab.Router.reopen({location: 'none'});

//**** Utility methods (for tests only - not for use in apps) ***

var lookupStore = function() {
  return StudioSabab.__container__.lookup('store:main');
}

var lookupRouter = function() {
  return StudioSabab.__container__.lookup('router:main');
}
...
beforeEach(function(done) {
  ...
  Ember.run(function() {
    ...
    StudioSabab.advanceReadiness();

    // When App readiness promise resolves, setup is complete
    StudioSabab.then(function() {
      done();
    });
  });
});
